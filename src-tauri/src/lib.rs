...
fn update_tray_menu(
    ...
    if let Some(tray) = app_handle.tray_by_id("main") {
        let play_pause_i =
            MenuItem::with_id(&app_handle, "play_pause", &play_pause, true, None::<&str>)
                .map_err(|e| e.to_string())?;
        let next_i = MenuItem::with_id(&app_handle, "next", &next, true, None::<&str>)
            .map_err(|e| e.to_string())?;
        let prev_i = MenuItem::with_id(&app_handle, "prev", &prev, true, None::<&str>)
            .map_err(|e| e.to_string())?;
        ...
        .setup(|app| {
            ...
            let _tray = TrayIconBuilder::with_id("main")
                ...
                .on_menu_event(move |app_handle, event| match event.id.as_ref() {
                    "play_pause" => {
                        let _ = app_handle.emit("tray_play_pause", ());
                    }
                    "next" => {
                        let _ = app_handle.emit("tray_next", ());
                    }
                    "prev" => {
                        let _ = app_handle.emit("tray_prev", ());
                    }
                    "show" => {
